...
size_t binary_tree_size(const binary_tree_t *tree)
{
	 ...
	 * (current node + left subtree size + right subtree size)
	 */
 ...
 *
 * A binary tree is considered perfect if all levels are
 * fully filled (i.e., all nodes at each level have two children)
 * and the number of nodes in the tree is equal to
 * 2^(height + 1) - 1, where 'height' is the height of the binary tree.
 * This function checks if the given binary tree
 * is perfect using the height and size.
 */
int binary_tree_is_perfect(const binary_tree_t *tree)
{
	...
	/**
	 * Calculate the perfect size of the binary tree using
	 * 2^(height + 1) - 1 formula
	 */
	perfect_size = (1 << (h + 1)) - 1;

	/**
	 * Check if the size of the binary tree matches the perfect
	 * size to determine if it's perfect
	 */
